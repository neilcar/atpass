...
import { login, DEFAULT_SERVICE } from "../lib/vaultClient";
import type { Agent } from "../lib/vaultClient";

export function LoginScreen({ onLoggedIn }: { onLoggedIn: (agent: Agent) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const agent = await login(service, handle.trim(), appPassword);
      onLoggedIn(agent);
    } catch (err) {
